<?php
	...
	?>
		...
		<form id="appsFrm" name="appsFrm" method="POST" action="customers.php">
			
			<input id="idUserFld" name="idUserFld" type="hidden" value="<?php echo $_GET["id"]; ?>">		
			
			<?php
									
			$select_customers_query = "SELECT id,category, 'true' as mark FROM type_user WHERE id 
IN 
(SELECT uc.tu_id FROM customer_category uc, type_user tu WHERE uc.tu_id = tu.id AND uc.endTime IS NULL AND uc.customer_id = " . $_GET["id"] . " ) 
UNION SELECT id,category, 'false' as mark FROM type_user WHERE id 
NOT IN 
(SELECT uc.tu_id FROM customer_category uc, type_user tu WHERE uc.tu_id = tu.id AND uc.endTime IS NULL AND uc.customer_id = " . $_GET["id"] . " ) ";
